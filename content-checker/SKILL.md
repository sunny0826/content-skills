...
description: >-
    当用户要求核查博客、审校 Markdown 文章、对比原文来源、检查事实准确性、指出翻译偏差、生成发布前改进建议，或外部编排层提交文章复核任务时使用。
...
- 调用方提供的 `source-*.txt` / `source-*.json` 或其它抽取结果：若同一轮已经生成，优先复用；核查缺关键上下文时再用轻量方式补充抽取。
...
- 如果参考资料来自 URL，优先使用调用方已经提供的 `source-*.txt` / `source-*.json`，或等价的正文抽取结果；不要把原始 HTML 打印到终端或对话中。
- **重点关注以下容易出错的区域：**
...
- **优先复用抽取结果**：若本次任务上下文中已存在 `source-*.txt` / `source-*.json` 或其它正文抽取结果，优先使用这些文件进行核查；不要重复 WebFetch 或再次抓取相同 URL，除非抽取结果缺失关键段落且无法核对。
